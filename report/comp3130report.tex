...
Because the entire game tree isn't computable, our negamax has a depth cutoff. At this cut off, it uses our static evaluation to determine how far in our favour the game board is. A small (but useful) optimisation to this process is increasing the search depth by 1 if we explore along a forced move - both because there is no branching that would increase computation cost, and this is an interesting board that we should explore further.
...
\hrule
  ...
  \item
    However this means all wins / losses are worth exactly the same even if we win or lose by more. this means if we see enemy has a win we give up rather than try to minimise the margin
  \end{itemize}
